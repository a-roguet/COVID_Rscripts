...
# select fields of interest
data.qPCR <- data.qPCR |> 
  dplyr::select(Dilution, analytic_comments, Run, Sample_ID, Target, Cт, NeedRerun)
...
info.WWTP<-read.xlsx("~/OneDrive - UWM/SARS-CoV-2/DATA//WWTP_Methods_Assays_info.xlsx", sheet = "method", colNames = TRUE)
...
info.WWTP<-read.xlsx("~/OneDrive - UWM/SARS-CoV-2/DATA//WWTP_Methods_Assays_info.xlsx", sheet = "assay", colNames = TRUE)
...
file.copy(from=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", rmd), to=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time, "/", gsub("_OPEN_KNIT", "_DO_NOT_MODIFY", rmd)), overwrite = TRUE, copy.mode = TRUE)

rmd.sample.selection<-list.files("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", pattern=glob2rx("DatabaseCuration_v2_DO_NOT_MODIFY*.Rmd"))
file.copy(from=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", rmd.sample.selection), to=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time, "/", gsub("_DO_NOT_MODIFY", "_OPEN_MODIFY_KNIT", rmd.sample.selection)), overwrite = TRUE, copy.mode = TRUE)
...
file.copy(from="~/OneDrive - UWM/SARS-CoV-2/DATA/WWTP_Methods_Assays_info.xlsx", to=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time, "/RawData/WWTP_Methods_Assay_info.xlsx"), overwrite = TRUE, copy.mode = TRUE)
...
# 1. Convert Ct into GC per reaction
  # Fix Ct
qPCR <- qPCR |> dplyr::mutate_at(c('Cт'), ~tidyr::replace_na(.,0)) # replace NA by 0
qPCR$Cт<-ifelse(qPCR$Cт=="Undetermined", 0, as.numeric(as.character(qPCR$Cт)))
  # GC per reaction: y = slope + Cт + intercept
qPCR$GC.per.rx<-10^((qPCR$Cт-qPCR$intercept)/(qPCR$slope))
...
write.table(final.report, paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time, "/uwm_report_v2_", current_time, ".tsv"), row.names = FALSE, quote = FALSE, sep = "\t", col.names = TRUE)
pander(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time, "/uwm_report_v2_", current_time, ".tsv"))
```
...
samples.to.ignore<-suppressWarnings(read.table("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/samples_to_ignore.txt", h=F))
