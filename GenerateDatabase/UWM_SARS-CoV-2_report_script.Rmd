...
file.copy(from=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", rmd), to=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time1, "/", rmd), overwrite = TRUE, copy.mode = TRUE)

rmd.sample.selection<-list.files("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", pattern="DatabaseCuration.Rmd")
file.copy(from=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", rmd.sample.selection), to=paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time1, "/", rmd.sample.selection), overwrite = TRUE, copy.mode = TRUE)
...
write.xlsx(report, paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time1, "/uwm_report_", current_time2, ".xlsx"))
paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/", current_time1, "/uwm_report_", current_time2, ".xlsx")
...
N1.plot<-ggplot(report, aes(x=as.Date(SampleCollectDate, format = "%m/%d/%Y") , y=N1SARSCoV2Conc))+
  ...
  facet_wrap(~WWTPName, scales = "free_y", ncol = 2) + 
...
pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/N1_concentration.pdf")); N1.plot; dev.off()
``` 
...
pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/N2_concentration.pdf")); N2.plot; dev.off()
```
```{r echo=FALSE, dpi=300}
...
pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/N1N2_concentration.pdf")); N1N2.plot; dev.off()
```
```{r echo=FALSE, dpi=300}
BCoV.plot<-ggplot(report, aes(x=as.Date(SampleCollectDate, format = "%m/%d/%Y"), y=BCoVRecRate))+
  geom_bar(position = position_dodge2 (width = 1), stat = "identity", colour="black", size=.3) + 
  facet_wrap(~WWTPName, scales = "free", ncol = 2) + 
  theme(axis.text.x = element_text(angle = 45, size = 4, hjust=1), 
        axis.title.x = element_blank()) + 
  ggtitle("BCoV recovery")
BCoV.plot

pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/BCoV_recovery.pdf")); BCoV.plot; dev.off()
```
...
pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/PMMoV_concentration.pdf")); PMMoV.plot; dev.off()
```
...
pdf(paste0("~/OneDrive - UWM/SARS-CoV-2/REPORTS/CDC_DHS_reports/Vizualization/HF183_concentration.pdf")); HF183.plot; dev.off()
```
