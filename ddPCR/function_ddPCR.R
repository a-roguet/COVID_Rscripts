Analyse_ddPCR_results <- function() {
  ...
  # four.clusters<-Artificial.sample.plate[["A01"]]
  # dropletPlot(four.clusters, cMethod="Cluster")
  # saveRDS(four.clusters, file = "~/OneDrive - UWM/SARS-CoV-2/DATA/ddPCR data/Artificial_Sample/four_clusters_variant681.RData")
  # 
  # 
  # 
  # 
  ...
  # four.clusters.variant681.genuine <- readRDS(file = "~/OneDrive - UWM/SARS-CoV-2/DATA/ddPCR data/Artificial_Sample/four_clusters_variant681.RData")
  # save(four.clusters.genuine, four.clusters.sludge.genuine, four.clusters.variant.genuine, four.clusters.variant944.genuine, four.clusters.variant681.genuine, file = "~/OneDrive - UWM/SARS-CoV-2/DATA/ddPCR data/Artificial_Sample/four_clusters_ALL.RData")
  ...
  ## Variant 681 - Get the center of the negative droplets
  four.clusters.variant681.genuine.NN <- four.clusters.variant681.genuine@dropletAmplitudes[which(four.clusters.variant681.genuine@classification$Cluster == "NN"),]
  four.clusters.variant681.genuine.NN.Ch1<-mean(four.clusters.variant681.genuine.NN$Ch1.Amplitude) 
  four.clusters.variant681.genuine.NN.Ch2<-mean(four.clusters.variant681.genuine.NN$Ch2.Amplitude) 
  #dropletPlot(four.clusters.variant681.genuine, cMethod="Cluster")
  ...
  for(j in 1:length(dataTempFiles)) {
    ...
    if(any(is.na(temp$Ch2.Amplitude))){
      ...
      write.table(temp, paste0("./temp_HEXsingleplex/", dataTempFiles[j]), quote = FALSE, sep = ",", col.names = TRUE, row.names = FALSE)
        ...
        } else if(grepl("681M_681W", samples[which(samples$Well == wellTemp), c("target")], fixed = TRUE)){
          variant681multiplex=variant681multiplex+1
          write.table(temp, paste0("./temp_variants681multiplex/", dataTempFiles[j]), quote = FALSE, sep = ",", col.names = TRUE, row.names = FALSE)
...
#####  Read the N1/N2 multiplex data - SLUDGE ###### 
  ...
  #####  Read the BioRad variant 681 multiplex data  ###### 
  if(variant681multiplex>0){
    print("variant 681 multiplex sample(s) detected")
    plate.variant681.genuine.multiplex <- ddpcrPlate(well="temp_variants681multiplex/.")
    No.variant681.samples<-length(names(plate.variant681.genuine.multiplex))
    names(plate.variant681.genuine.multiplex) # list all the N1/N2 wells
    plate.variant681.cluster <- plate.variant681.genuine.multiplex # duplicate "plate.sludge.genuine". "plate.sludge.cluster" will store the k-means clusters
    list.variant681<-matrix(NA, No.variant681.samples, 1)
    
    #commonClassificationMethod(plate.variant681.genuine.multiplex) # 
    #facetPlot(plate.variant681.genuine.multiplex, cMethod="Cluster") # droplets per well
    
    
    ## Define 4 clusters (i.e. NN (negative), PN (N1), PP (N1/N2) and NP (N2) for each well)
    for(i in 1:No.variant681.samples) {
      
      ## Preparation
      well = 0
      four.clusters.NN.Ch1 = four.clusters.variant681.genuine.NN.Ch1
      four.clusters.NN.Ch2 = four.clusters.variant681.genuine.NN.Ch2
      four.clusters <- four.clusters.variant681.genuine
      well_ID=as.character(names(plate.variant681.genuine.multiplex)[i])
      
      well<-plate.variant681.genuine.multiplex[[well_ID]]
      well_sample<-as.character(samples[which(samples$Well == well_ID), 2])
      well_target<-as.numeric(gsub("([0-9]+).*$", "\\1", strsplit(as.character(samples[which(samples$Well == well_ID), 3]), '_' ,fixed=TRUE)[[1]][1]))
      well_sample_plot<-str_replace(well_sample, ":", "to")
      well_sample_plot<-str_replace(well_sample_plot, "/", "-")
      ...
      ## Define the 4 clusters (NN (negative), PN (N1), PP (N1/N2) and NP (N2))
      well <- kmeansClassify(well, 
                             centres=matrix(c(600-ch1, 1000-ch2,    4000-ch1, 1000-ch2,   4000-ch1, 4000-ch2,   600-ch1, 4000-ch2), ncol=2, byrow=TRUE))
      dropletPlot(well, cMethod="kmeans")
      ...
      ## Add info to the plate
      plate.variant681.cluster[[well_ID]]<-well
    ...
    ## Generate multiplex data
    print(paste0(No.variant681.samples, " variant 681 files processed"))
    cat(sep="\n\n")
    print("Compiling the output files...")
    results.variant681.multiplex<-plateSummary(plate.variant681.cluster, cMethod="kmeansSdRain", ch1Label = "M", ch2Label = "W")
    results.variant681.multiplex$FAM.HEX.difference<-round(results.variant681.multiplex[,10]/results.variant681.multiplex[,11], digits = 2)
    results.variant681.multiplex$flag.FAM.HEX.difference<-"okay"
    results.variant681.multiplex$flag.FAM.HEX.difference[is.na(results.variant681.multiplex$flag.FAM.HEX.difference)] <- "okay"
    
    
    ## Split data to get 1 row = 1 assay
    results.variant681.multiplex.FAM<-results.variant681.multiplex[, c(1,2,5,6,7,10,12,18)]
    results.variant681.multiplex.FAM$Target<-paste0(list.variant681, "M")
    results.variant681.multiplex.FAM$DyeName<-"FAM"
    results.variant681.multiplex.FAM$Well<-row.names(results.variant681.multiplex.FAM)
    results.variant681.multiplex.HEX<-results.variant681.multiplex[, c(1,3,5,8,9,11,13,18)]
    results.variant681.multiplex.HEX$Target<-paste0(list.variant681, "W")
    results.variant681.multiplex.HEX$DyeName<-"HEX"
    results.variant681.multiplex.HEX$Well<-row.names(results.variant681.multiplex.HEX)
    
    results.variant681.multiplex.export<-rbind(results.variant681.multiplex.FAM,setnames(results.variant681.multiplex.HEX,names(results.variant681.multiplex.FAM)))
    names(results.variant681.multiplex.export)<-c("PP", "PN_NP", "AcceptedDroplets", "PositivesDroplets", "NegativesDroplets", "Conc(copies/µL)", "Copies/20µLWell", "Flag.FAM_HEX.difference", "Target", "DyeName", "Well")
    results.variant681.multiplex.export$Flag.positive.droplets<-ifelse(results.variant681.multiplex.export$PositivesDroplets/results.variant681.multiplex.export$AcceptedDroplets>0.7,paste0("too many positive droplets (", round(results.variant681.multiplex.export$PositivesDroplets/results.variant681.multiplex.export$AcceptedDroplets*100, digits = 0), ")"), "okay")
    results.variant681.multiplex.export$Flag.total.droplets<-ifelse(results.variant681.multiplex.export$AcceptedDroplets<=10000,"low number of droplets", "okay")
    results.variant681.multiplex.export$Run<-run_ID
    results.variant681.multiplex.export$Comment<-""
    #print(paste0("Number of row before sample name transformation: ", nrow(results.variant681.multiplex.export)))
    results.variant681.multiplex.export<-setDT(samples)[results.variant681.multiplex.export, on="Well"]
    #print(paste0("Number of row after sample name transformation (should the same): ", nrow(results.variant681.multiplex.export)))
    results.variant681.multiplex.export<-results.variant681.multiplex.export[,c("Run", "Well", "Sample", "Target", "Conc(copies/µL)", "DyeName", "Copies/20µLWell", "PP", "PN_NP", "AcceptedDroplets", "PositivesDroplets", "NegativesDroplets", "Flag.positive.droplets", "Flag.total.droplets", "Flag.FAM_HEX.difference", "Comment")]
    results.variant681.multiplex.export$NeedRerun<-ifelse(results.variant681.multiplex.export$Flag.positive.droplets=="okay" & results.variant681.multiplex.export$Flag.total.droplets == "okay" & (results.variant681.multiplex.export$Flag.FAM_HEX.difference<4 | results.variant681.multiplex.export$Flag.FAM_HEX.difference=="okay") & results.variant681.multiplex.export$Comment == "", "", "need_rerun")
    results.variant681.multiplex.export<-results.variant681.multiplex.export[,c("Flag.positive.droplets", "Flag.total.droplets", "Flag.FAM_HEX.difference", "Comment", "NeedRerun", "Run", "Well", "Sample", "Target", "Conc(copies/µL)", "DyeName", "Copies/20µLWell", "PP", "PN_NP", "AcceptedDroplets", "PositivesDroplets", "NegativesDroplets")]
    # If there is only 1 sample, it is to avoid the Well to be called 1, and Sample to be called NA
    if(nrow(results.variant681.multiplex.export)==2){
      results.variant681.multiplex.export[, c("Well")]<-well_ID
      results.variant681.multiplex.export[, c("Sample")]<-well_sample
    }
    
    #Export summarized data for BioRad variant681 assays
    ...
    ## Export the data ## 
    write.table(results.variant681.multiplex.export, paste0("results_variant681.csv"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = ",")
    write.table(results.variant681.multiplex, paste0("details_results_variant681.csv"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = ",")
    saveRDS(plate.variant681.cluster, file = "plate_variant681.RData")
...
if(BCoVBRSVmultiplex>0){
  ...
  final.variant.results <- rbind(if(exists("results.variant944.multiplex.summarized")) results.variant944.multiplex.summarized,
                                 if(exists("results.variant681.multiplex.summarized")) results.variant681.multiplex.summarized,
