...
print(paste0(0, " raw amplification files detected"))
print(paste0(""))
print(paste0(0, " raw amplification files detected"))
...
save_info<- function() {
  plate.cluster[[well_ID]]<-well
  samples<-data.table::fread(samples.info, fill=TRUE, select=c("Well", "Sample"))
  well_sample<-as.character(samples[which(samples$Well == well_ID), 2])
  pdf(paste0("multiplex_", well_ID, "_", well_sample, "_2D_currated.pdf"), width=8, height=8)
  print(dropletPlot(well, cMethod="kmeansSdRain"))
  dev.off(); dev.off()
}
...
export_multiplex_curated_data<- function() {
  ...
  ## Generate multiplex data
  results.multiplex<-plateSummary(plate.cluster, cMethod="kmeansSdRain", ch1Label = multiplex_assay_FAM, ch2Label = multiplex_assay_HEX)
  results.multiplex$FAM.HEX.difference<-round(results.multiplex[,10]/results.multiplex[,11], digits = 2)
  results.multiplex$flag.FAM.HEX.difference<-ifelse(results.multiplex$FAM.HEX.difference>2,results.multiplex$FAM.HEX.difference, "okay")
  
  ## Split data to get 1 row = 1 assay
  results.multiplex.FAM<-results.multiplex[, c(1,2,5,6,7,10,12,18)]
  results.multiplex.FAM$Target<-multiplex_assay_FAM
  results.multiplex.FAM$DyeName<-"FAM"
  results.multiplex.FAM$Well<-row.names(results.multiplex.FAM)
  results.multiplex.HEX<-results.multiplex[, c(1,3,5,8,9,11,13,18)]
  results.multiplex.HEX$Target<-multiplex_assay_HEX
  results.multiplex.HEX$DyeName<-"HEX"
  results.multiplex.HEX$Well<-row.names(results.multiplex.HEX)
  ...
  ## Export the data ## 
  write.table(results.multiplex.export, paste0("results_N1N2_curated.csv"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = ",")
  write.table(results.multiplex, paste0("results_details_N1N2_curated.csv"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = ",")
  saveRDS(plate.cluster, file = "plate_N1N2_currated.RData")
...
save_info_singleplex<-function() {
  ## Print plots
  well.plot<-as.data.frame(cbind(well@dropletAmplitudes$Ch1.Amplitude, well@classification$threshold))
  names(well.plot)<-c("Ch1.Amplitude", "thresholds")
  well.plot$thresholds<-gsub(1, "negative", well.plot$thresholds)
  well.plot$thresholds<-gsub(3, "positive", well.plot$thresholds)
  samples<-data.table::fread(samples.info, fill=TRUE, select=c("Well", "Sample"))
  well_sample<-as.character(samples[which(samples$Well == well_ID), 2])
  pdf(paste0("singleplex_", well_ID, "_", well_sample, "_curated.pdf"), width=8, height=8)
  print(ggplot(well.plot, aes(runif(nrow(well.plot),1,nrow(well.plot)), Ch1.Amplitude, colour = as.factor(thresholds))) + 
          geom_point())
  dev.off(): dev.off()
  ## Add info to the plate
  well@classification$Cluster<-well@classification$thresholds
  plate.genuine.FAMsingleplex[[well_ID]]<-well
}
...
export_singleplexFAM_curated_data<-function(){
  ...
  write.table(results.FAMsingleplex.export, paste0("results_", singleplex_FAMassay, "_FAMsingleplex_curated.csv"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = ",")
  write.table(results.FAMsingleplex, paste0("results_details_", singleplex_FAMassay, "_FAMsingleplex_curated.csv"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = ",")
  saveRDS(plate.genuine.FAMsingleplex, file = paste0("plate_", singleplex_FAMassay, "_FAMsingleplex_curated.RData")) 
